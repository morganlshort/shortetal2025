...
# Create New dataset for Coral Tissues only #############
FECTP <- subset(Trophic_Position_10_6_24, Tissue_type=='coralt')
#Fecundity & Trophic position
FECTP
# Summary Statistics for CORALS at each site CLEAN ############
...
# Summary Statistics for ENDMEMBERS at each site CLEAN ############################
...
# Coral Trophic Position ~ Site One-Way ANOVA (Glu-Phe) CLEAN ######### ###################
class(FECTP$Site)
...
# All Tissues' Trophic Position ~ Tissue & Site Boxplot CLEAN #############################
...
# All Tissues' Source (phe & lys) ~ Tissue & Site Boxplot CLEAN ###############################
...
#Legend
par(new=TRUE, xpd = TRUE) #xpd = TRUE expands the area of the plot so that the legend can sit outside of the scatterplot
...
# Create New datasets for Coral Tissues split by Male & Female ###################
...
FECTPF#female SSIDs
#9 total
# Summary Stats for Trophic Position ~ Fertility or M/F CLEAN ####
...
# Trophic Position ~ Fertility t-TEST CLEAN #####################
...
# Trophic Position ~ Sex Wilcoxon-Rank-Sum Test CLEAN ##########################
...
# Female Fecundity ~ Trophic Position Correlation Analysis CLEAN ###########################
attach(FECTPF)
library(lme4)
library(car)
...
#Summary plot
lm(Cross_Mes~TP_Glu_Phe)
# Coefficients:
#   (Intercept)   TP_Glu_Phe  
#     14.421        5.581  
windows(8,8)
plot(Cross_Mes~TP_Glu_Phe,pch=16,axes=F,xlab='',ylab='')
axis(1,cex.axis=0.8)
mtext(text='Trophic Position (Glu-Phe)',side=1,line=3)
axis(2,las=1)
mtext(text='No. ova per polyp',side=2,line=3)
box(bty='l')
abline(lm(Cross_Mes~TP_Glu_Phe))
x <- seq (min(TP_Glu_Phe),max(TP_Glu_Phe),l=1000)
y <- predict (lm(Cross_Mes~TP_Glu_Phe),data.frame(TP_Glu_Phe=x),interval='c')
matlines(x,y,lty=2,col=1)
detach(FECTPF)

# Male Fecundity ~ Trophic Position Correlation Analysis CLEAN ###########################
attach(FECTPM)
library(lme4)
library(car)
scatterplot(Cross_Mes,TP_Glu_Phe)
...
detach(FECTPM)
    ...
    #FECUNDITY ~ TROPHIC POSITION GRAPHS (M/F) CLEAN ###########################
    #MALES (ALL SITES) CLEAN #################
windows(8,8)
par(mar = c(5, 6, 4, 3) + 0.5)             

# Assign different shapes for each site
pch_values <- c(17, 19, 15, 8)  # triangle, circle, square, star
...
# Map the factor levels to point shapes for plotting
...
abline(reg, lwd = 3)     
mtext("No. Spermaries per Polyp", side = 2, line = 4.5, cex = 2.0)
...
plot(FECTPF$TP_Glu_Phe, FECTPF$Cross_Mes, pch = pch_values[site_shapes],
     ...
     las =1)
mtext("No. Eggs per Polyp", side = 2, line = 4.5, cex = 2.0)
reg<-lm(Cross_Mes ~ TP_Glu_Phe, data = FECTPF) 
abline(reg, lwd = 3)
mtext(expression('TP'[Glu-Phe]), side = 1, line = 3, col='Black', cex = 2.0)
attach(FECTPF)
arrows(x0=TP_Glu_Phe-TP_prop_uncertainty, y0=Cross_Mes, x1=TP_Glu_Phe+TP_prop_uncertainty, y1=Cross_Mes, code=3, angle=90, length=0.05)
detach(FECTPF)
...
# %Heterotrophy ~ Site One-Way ANOVA CLEAN ################
